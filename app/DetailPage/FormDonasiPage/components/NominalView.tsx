...
export default function NominalView({ campaign, amount, setAmount, onNext, onBack }: NominalViewProps) {
  ...
  const presetNominals = [25000, 50000, 75000, 100000, 200000, 500000];
  ...
  return (
    <div className="w-full max-w-md mx-auto bg-white min-h-screen flex flex-col relative animate-in slide-in-from-right-4 duration-300">
      ...
      <div className="bg-white/80 backdrop-blur-md px-5 py-4 flex items-center gap-4 sticky top-0 z-20 border-b border-slate-100">
        ...
        <h1 className="font-bold text-slate-800 text-[15px] tracking-wide truncate">
          {campaign?.title || "Nominal Donasi"}
        </h1>
      </div>
      ...
      <div className="p-6 flex-1 overflow-y-auto pb-32">
        ...
        <div className="flex flex-col items-center justify-center pt-8 pb-10">
          ...
          <div className="flex items-center gap-1 justify-center border-b-2 focus-within:border-purple-600 transition-colors pb-2 min-w-[200px]">
            ...
            <input 
              ...
              className="bg-transparent text-center font-black text-4xl outline-none text-purple-600 placeholder-slate-200 w-full max-w-[200px]" 
              placeholder="0" 
            />
          </div>
            ...
            {presetNominals.map((val) => (
              <button 
                ...
                className={`py-4 px-2 rounded-2xl border-2 font-bold text-[14px] transition-all duration-200 ${
                  amount === val 
                    ? "border-purple-600 bg-purple-600/5 text-purple-600 shadow-sm" 
                    : "border-slate-100 bg-white text-slate-600 hover:border-purple-600/30"
                }`}
              >
                {formatRp(val)}
              </button>
        ...
        <button 
          ...
          className={`w-full font-bold text-[15px] py-4 rounded-2xl transition-all pointer-events-auto ${
            !amount || Number(amount) < 1000
              ...
              : "bg-gradient-to-r from-[#7C3996] to-[#5B2A73] text-white shadow-xl shadow-purple-600/20 hover:bg-purple-700 active:scale-[0.98]"
          }`}
        >
          Lanjutkan
        </button>
